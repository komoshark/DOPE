...
#optimal policy from pre-trained model
...
#safe baseline policy from pre-trained model
...
for sim in range(NUMBER_SIMULATIONS):
    ...
    for episode in range(NUMBER_EPISODES):
        if episode <= K0:
            #use baseline policy for first K0 episodes
            pi_k = pi_b
            val_k = val_b
            cost_k = cost_b
            q_k = q_b
            ...
            util_methods.compute_confidence_intervals(L, 1) #compute confidence bound beta, 1 for episode <= K0 
        else:
        ...
        if episode != 0 and episode%5000 == 0:
            ...
            print('Epoch {:d} Duration {:.3f} seconds.'.format(episode, duration))
