...
class utils:
    ...
    def setCounts(self,ep_count_p,ep_count):
        '''
        set number of times (s, a) and (s,a,s') were observed.
        '''
        for s in range(self.N_STATES):
            for a in self.ACTIONS[s]:
                self.NUMBER_OF_OCCURANCES[s][a] += ep_count[s, a] #number of times the pair (s, a) was observed
                for s_ in range(self.N_STATES):
                    self.NUMBER_OF_OCCURANCES_p[s][a, s_] += ep_count_p[s, a, s_] #number of times the pair (s, a, s_) was observed
